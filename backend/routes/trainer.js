...
// input validation
const trainerSignupSchema =zod.object({
        email:zod.string().email(),
        firstName:zod.string(),
        lastName:zod.string(),
        password:zod.string(),
})
const trainerSigninSchema =zod.object({
    email:zod.string().email(),
    password:zod.string(),
})
const trainerUpdateSchema =zod.object({
        firstName:zod.string().optional(),
        lastName:zod.string().optional(),
        password:zod.string().optional(),
})

trainerRouter.post('/signup',async function (req,res) {
    const inputFromUser={
...
trainerRouter.post("/signin",async function (req,res) {
    const userInput ={
        email:req.body.email,
        password:req.body.password,
    }
...
trainerRouter.put("/",authMiddleware,async function (req,res) {
...
trainerRouter.get("/me",authMiddleware,async function (req,res) {
  const user=  await User.findOne({
        _id:res.userId
    })
    res.status(200).json({
